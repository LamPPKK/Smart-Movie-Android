...
This document defines what “supported” means for each Android device class in SmartMovie 2.0. It is deliberately stricter than a manifest compatibility claim: a platform is listed as supported only when its input model, layout, packaging, and release behavior are represented in the codebase.
