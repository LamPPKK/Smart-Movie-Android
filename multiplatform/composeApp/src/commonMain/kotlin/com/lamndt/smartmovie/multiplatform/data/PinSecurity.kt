...
internal fun pinDigest(salt: String, pin: String): String = sha256("$salt:$pin".encodeToByteArray())
    .joinToString("") { it.toUByte().toString(16).padStart(2, '0') }

// Small common-code SHA-256 implementation keeps the local PIN format identical
// on JVM, JavaScript and Wasm without persisting the PIN or relying on platform UI.
private fun sha256(input: ByteArray): ByteArray {
    ...
    return ByteArray(32).also { result ->
        hash.forEachIndexed { index, value ->
            result[index * 4] = (value ushr 24).toByte(); result[index * 4 + 1] = (value ushr 16).toByte()
            result[index * 4 + 2] = (value ushr 8).toByte(); result[index * 4 + 3] = value.toByte()
        }
    }
}
